...
#define M_PI 3.1415f
#define OUTPUT_YAW_PITCH_ROLL 1
...
void imu_init(void){
    ...
    */
    if(devStatus == 0) {
        SEGGER_RTT_printf(0,"Enabling DMP...\r\n");
        mpu6500SetDMPEnabled(true);
        
        SEGGER_RTT_printf(0,"Enabling interrupt detection...\r\n");
        attach_interrupt();
        mpuIntStatus = mpu6500GetIntStatus();
        
        SEGGER_RTT_printf(0,"DMP ready! Waiting for first interrupt...");
        dmpReady = true;
        
        // Get expected DMP packet size for later comparison
        packetSize = mpu6500DmpGetFIFOPacketSize();
    } else {
        SEGGER_RTT_printf(0,"DMP Initialization failed (code: %d)\r\n",devStatus);
    }
}
...
void imu_periodic(void){
    ...
    // otherwise, check for DMP data ready interrupt (this should happen frequently)
            ...
            mpu6500DmpGetEuler(ypr, &q);
			mpu6500DmpGetEulerPhi(&ypr[2], &q);
